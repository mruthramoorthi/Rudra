...
require("jspdf-autotable");
 ...
 router.get("/savings", async (req, res) => {
    const enroll = await backendController.selectQuery(`SELECT * FROM customer_savings_enroll where deleteon=? and branch=? and id=?`,['0000-00-00',userToken.site,userToken.userid]);
    const numbers = enroll.map(item => item.savings_number);  // Extract roles
    backendController.selectQuery(`SELECT * FROM scheme_group_count where deleteon=? and branch=? and savingsnumber in (?)`,['0000-00-00',userToken.site,numbers])
    .then(results => {
        let options_arr = [];
        results.forEach(item => {
            options_arr.push(`<option value="${item["savingsnumber"]}">${backendController.allCaps(item['schemegroup'])} - ${backendController.allCaps(item['savingsnumber'])} ( ${item['savingscount']} )</option>`);
        });
 ...
 router.post("/select", async (req, res) => {
    ...
    backendController.selectQuery(`SELECT * FROM customer_savings_enroll where deleteon=? and branch=? and savings_number=?`,['0000-00-00',userToken.site,number])
        .then(results => {
            const html = generateHTMLData(results,controls,savings,rate,scheme);
...
let sno = 0;
function generateHTMLData(results,controls,savings,rate,scheme) {
    let htmlContent = '';
    const entryDate = backendController.rawDateFormat(savings[0].entrydate);
    ...
    results.forEach(item => {
        ...
        if(scheme[0].type=="gram"&&scheme[0].category!='cash'&&item.closed=="0")
        {
            gram_or_cash_conditions = `<label>You Have to Pay ${backendController.formatNumberWithCommas(item.due)} for Save ${backendController.caps(scheme[0].category)}[${backendController.formatNumberWithCommas(rate[0]["price"])}] ${backendController.money(parseFloat(item.due)/parseFloat(rate[0]["price"]), 3)} G</label><br>
            <input type="hidden" id="rate" name="rate" value="${rate[0]["price"]}">
            <input type="hidden" id="gram" name="gram" value="${backendController.money(parseFloat(item.due)/parseFloat(rate[0]["price"]), 3)}">`;
        }
        htmlContent += `<div class="snowdiv"><h3>${backendController.caps(item.savingname)}</h3>
        ...
        if(item.closed=="0" && savings[0])
        {
            ...
            if((nextduedate<=today || controls[0].paypermonth>savings[0].restrict) && savings.length<controls[0].month)
            {
                ...
                htmlContent += `<input type="hidden" id="realdue" name="realdue" value="${item.due}">
                ...
                ${htmls.createInputElement({type:"number" ,id:"due", placeholder:"Cash Due", value:item.due, othersForInput:"readonly"})}
        ...
        else if(!savings[0])
        {
            ...
            <input type="hidden" id="realdue" name="realdue" value="${item.due}">
        ...
        htmlContent += `<br><br><label class="see-transaction">See Transactions</label><br><div id="transactions"><table><thead><tr><th>S.no</th><th>Date</th><th>Due</th><th>Rate</th><th>Gram</th><th>Cash</th><th>Bank</th></tr></thead><tbody>`;
        ...
        savings.forEach(item => {
            ...
            htmlContent += `<tr><td>${ssno}</td><td>${entryDate}</td><td>${backendController.money(item.due,0,1)}</td><td class='right-align'>${backendController.money(item.rate,0,1)}</td><td class='right-align'>${backendController.money(item.gram,3,1)}</td><td class='right-align'>${backendController.money(item.cashamount,0,1)}</td><td class='right-align'>${backendController.money(item.bankamount,0,1)}</td></tr>`;
            bankAmt += item.bankamount;
            cashAmt += item.cashamount;
            gram += item.gram;
        });
        htmlContent += `</tbody><tfooter><td colspan='4'>Total</td><td class='right-align'>${backendController.money(gram,3,1)}</td><td class='right-align'>${backendController.money(cashAmt,0,1)}</td><td class='right-align'>${backendController.money(bankAmt,0,1)}</td><td></td></tfooter></table>`;
...
router.post("/insert", async (req, res)=>{
    const {realdue, paymenttype, due, banks, transactiontype, bank, savinggrp, savingnumbers, savingscount, duemonth,duedate, restrict, savingtype, savingcategory, next_due_date, savingname, savingph1} = req.body;
    let gram = 0;
    let rate = 0;
    ...
    try
    {
        ...
        const insertResponse = await backendController.insert({body: {tableName: 'savings_transactions', data:{due,savinggroup:savinggrp, savingnumber:savingnumbers, savingcount:savingscount, cashamount:due, bankamount:bank, bank:banks, via:transactiontype, amountin:paymenttype, duedate, nextduedate:next_due_date, duemonth, restrict, gram, rate}}});
        if(due>0)    
