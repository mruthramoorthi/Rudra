...
router.post("/select", async (req, res) => {
   const totalRecode = await backendController.selectQuery(`SELECT count(*) as total FROM customer_savings_enroll where deleteon=? and branch=? and ob=?`,['0000-00-00',userToken.site,0]);
   backendController.selectQuery(`SELECT * FROM customer_savings_enroll where deleteon=? and branch =? and ob=?`,['0000-00-00',userToken.site,0])
       .then(results => {
...
router.post('/insert',async (req, res) => {
    const { customer, group, customername, ph1, ph2, address, uniquekey, savingcount, savingnumbers,attender} = req.body;
    if(group==="nd")
    ...
    if(!uniquekey)
    {
        number = scheme_details[0].savings_number_starting+""+ parseInt(backendController.valNum(count_of_chit[0].count) + 1);
        count = parseInt(backendController.valNum(chit_count[0].count) + 1);
    }
    const data = { name:customer_details[1],savingname: customername, schemegroup:scheme_details[0].schemegroup, ph1, ph2, address, id:customer_details[0], savings_number:number, savings_count:count, month:scheme_details[0].month, schemecode:group, totalamount:scheme_details[0].totalamount, due:scheme_details[0].due, attender };
    if (uniquekey)
    {
        try {
            // Call the `deleted()` function with the necessary arguments
    ...
    try
    {
        ...
        const insertResponse1 = await backendController.insert({body: {tableName: 'scheme_group_count',data: { savingsnumber:number, savingscount:count, schemecode:group, schemegroup:scheme_details[0].schemegroup }}});
