...
router.post('/insert',async (req, res) => {
    ...
    const exist_users = await backendController.selectQuery(`SELECT * FROM users where deleteon=? and (primaryphonenumber=? or secondaryphonenumber=?) and uniqueid!=?`,['0000-00-00',number1,number1,uniquekey]);
    const exist_num2 = await backendController.selectQuery(`SELECT * FROM users where deleteon=? and (primaryphonenumber=? or secondaryphonenumber=?) and uniqueid!=?`,['0000-00-00',number2,number2,uniquekey]);
    const exist_aadhar = await backendController.selectQuery(`SELECT * FROM users where deleteon=? and aadhaar=? and uniqueid!=?`,['0000-00-00',aadhaar,uniquekey]);
    const exist_pan = await backendController.selectQuery(`SELECT * FROM users where deleteon=? and pan=? and uniqueid!=?`,['0000-00-00',pannum,uniquekey]);
    ...
    else if (exist_aadhar[0]) {
        return res.json({msg:"Aadhaar Number Already Exist", msg_type : 'error', success: false });
    }
    else if (!aadhaar || aadhaar.length != 12 || !/^\d+$/.test(aadhaar)) {
        return res.json({msg:"Aadhaar Number Invalid", msg_type : 'error', success: false });
    }
    else if (exist_pan[0]) {
        return res.json({msg:"PAN Number Already Exist", msg_type : 'error', success: false });
    }
    else if (!pannum || pannum.length != 10) {
        return res.json({msg:"PAN Number Invalid", msg_type : 'error', success: false });
    }
    else if (!imgpath_profile) {
        return res.json({msg:"Profile is Not Valid", msg_type : 'error', success: false });
    }
    else if (!imgpath_proof) {
        return res.json({msg:"Proof is Not Valid", msg_type : 'error', success: false });
    }
