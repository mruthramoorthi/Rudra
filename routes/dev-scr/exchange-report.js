...
router.post("/pdf", async (req, res) => {
    ...
    try {
        ...
        const results = await backendController.selectQuery(
            `SELECT * FROM exchange where deleteon=? and branch=? and date between ? and ? and metal=? order by uniqueid desc`,
        ...
        doc.text("Exchnage Report", centerX, 10, { align: "center" });
        ...
        for (let i = 0; i < results.length; i++) {
            const exchange = results[i];

            sno++;
            const entryDate = backendController.rawDateFormat(exchange.date, 'dmy');

            // Calculate totals
            filteredRows.push([
                sno, 
                entryDate,
                backendController.caps(exchange.category), 
                backendController.caps(exchange.id), 
                backendController.money(exchange.tagno, 0, 1), 
                backendController.money(exchange.exweight, 3, 1), 
                backendController.money(exchange.price, 3, 1), 
            ]);
        }

        // Set table columns
        const columns = ["#", "Date", "Category", "ID", "Tag", "Weight", "Amount"];
...
router.post("/select", async (req, res) => {
    ...
    try{
        const totalRecode = await backendController.selectQuery(
            `SELECT count(*) as total FROM exchange where deleteon=? and branch=? and date between ? and ? and metal=?`,
            ['0000-00-00', userToken.site,fdate, todate,metal]
        );
...
async function generateHTMLData(groupedResults) {
    ...
    for (let i = 0; i < groupedResults.length; i++) {
            const exchange = groupedResults[i];
            sno++;
        htmlContent += `
            <tr>
                <td>${sno}</td>
                <td>${backendController.rawDateFormat(exchange.date,'dmy','-')}</td>
                <td>${backendController.caps(exchange.category)}</td>
                <td>${backendController.caps(exchange.id)}</td>
                <td class='right-align'>${backendController.caps(exchange.tagno)}</td>
                <td class='right-align'>${backendController.money(exchange.exweight,3,1)}</td>
                <td class='right-align'>${backendController.money(exchange.price, 3, 1)}</td>
            </tr>`;
            weight += parseFloat(exchange.weight);
            amount += parseFloat(exchange.oldamount);
    }
    htmlContent += `
        [tdfooter]<tr><td colspan='5'>Total</td><td class='right-align'>${backendController.money(weight,3,1)}</td><td class='right-align'>${backendController.money(amount,3,1)}</td></tr>`;
    sno = 0;
    return htmlContent;
}
